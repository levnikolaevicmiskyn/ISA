...
This is because appending another register instead of a latch will increase the overall latency; while selectively enabling the internal registers complicates their internal structures. Furthermore, the input latch will also perform a gating function (\textbf{guarded evaluation}), preventing the waste of power in the internal combinational nodes when the input samples are not valid.
